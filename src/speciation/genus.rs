...
use crate::speciation::species::Indiv;
...
impl<I: Individual<F>, F: num::Float> Genus<I, F> {
            ...
            MutateF: FnMut(&mut I),
    {
        ...
        for (species_i, species) in self.species_collection.iter().enumerate() {
            ...
            let mut new_individuals: Vec<I> = Vec::with_capacity(offspring_amounts[species_i]);
            for _n_offspring in 0_usize..offspring_amounts[species_i] {
                //let new_individual: I = self.generate_new_individual::<std::iter::Map<Iter<'_, Indiv<I, F>>, fn(&'_ Indiv<I, F>) -> &I>,SelectionF,ParentSelectionF,ReproduceI1F,CrossoverI2F,MutateF>(
                let new_individual: I = self.generate_new_individual::<_,SelectionF,ParentSelectionF,ReproduceI1F,CrossoverI2F,MutateF>(
                    conf,
                    species.iter().into(), // as Box<dyn ExactSizeIterator<Item=&I>>,
                    selection,
                    parent_selection,
    ...
    where
        ...
        MutateF: FnMut(&mut I),
    {
        let parent_pool_size: usize = population.as_mut().len();
        assert!(!parent_pool_size.is_empty());

        // Crossover
        let mut child: I =
            if conf.crossover && parent_pool_size > 1 {
                let parents = parent_selection(population);
